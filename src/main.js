...
var store = new Vuex.Store({
  state: {// this.store.state.***
    car: car // 将购物和的商品的数据，用一个数组存储起来，在 car 数组中，存储一些商品的对象，咱们可以暂时将这个商品对象设计成这个样子 { id: 商品的id, count: 要购买的数量, price: 商品的单价, selected: false}
  },
  mutations: {// this.$store.commit('方法名称','按需传递唯一的参数')
     ...
     updateGoodsSelected(state, info){
        ...
        // 把最新的购物车商品的状态，保存到 本地储存中
        localStorage.setItem('car', JSON.stringify(state.car))
     }
  },
  getters: {// this.$store.getters.***
    // 相当于 计算属性，相当于 filters
    getAllCount(state) {
      var c = 0;
...
Vue.use(VueResource)
// 设置请求的根路径
Vue.http.options.root = 'http://localhost:3500';
